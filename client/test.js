...
describe('EC2 설정을 확인합니다.', () => {
  it('EC2 연결이 되었다면, 서버의 응답으로 "서버 실행중"를 받아야 합니다.', async () => {
    const res = await axios.get(`${process.env.REACT_APP_API_URL}`);
    expect(res.data).to.be.eql('서버 실행중');
  });
});

describe('S3 설정을 확인합니다.', () => {
...
describe('데이터 베이스 연결 확인', () => {
  it('RDS를 배포하고, EC2와 연결했으면, "/status" 요청에 대한 응답이 { isLogin: true, isConnectedToDatabase: true } 이어야 한다', async () => {
